...
// ─── HELPERS ──────────────────────────────────────────────────────────────────
function darkSlide(slide: any) { slide.background = { color: C.night }; }
function lightSlide(slide: any) { slide.background = { color: C.pureWhite }; }

function sectionTag(slide: any, text: string, y = 0.38, pres: any) {
...
function slideTitle(slide: any, text: string, color = C.pureWhite, y = 0.78) {
  slide.addText(text, { x: 0.5, y, w: 9, h: 0.7, fontSize: 32, bold: true, color, fontFace: "Trebuchet MS" });
}

function statBox(slide: any, val: string, label: string, x: number, y: number, pres: any, w = 2.0) {
  slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
...
function featureRow(slide: any, icon: string, title: string, desc: string, y: number, pres: any) {
  slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
...
function planCard(slide: any, name: string, price: string, features: string[], x: number, pres: any, highlight = false) {
...
export const downloadPitchDeck = async () => {
  ...
  // ─── SLIDE 1 — CAPA ───────────────────────────────────────────────────────────
  {
    ...
    darkSlide(s);
    s.addShape((pres as any).shapes.RECTANGLE, { x: 0, y: 0, w: 0.06, h: 5.625, fill: { color: C.green }, line: { color: C.green } });
    s.addShape((pres as any).shapes.RECTANGLE, { x: 6.2, y: 0, w: 3.8, h: 5.625, fill: { color: C.nightMid }, line: { color: C.nightMid } });
    ...
    s.addShape((pres as any).shapes.RECTANGLE, { x: 0.5, y: 3.3, w: 1.2, h: 0.04, fill: { color: C.green }, line: { color: C.green } });
    ...
    ["R$120k MRR", "7 módulos", "4 especialidades"].forEach((t, i) => {
      ...
      if (i < 2) s.addShape((pres as any).shapes.LINE, { x: 7, y: 2.05 + i * 0.85, w: 2.2, h: 0, line: { color: "1E3A5F", width: 0.5 } });
    });
  ...
  // ─── SLIDE 2 — PROBLEMA ───────────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    lightSlide(s);
    sectionTag(s, "Problema", 0.38, pres);
    ...
    pains.forEach(([icon, title, desc], i) => featureRow(s, icon, title, desc, 1.65 + i * 0.82, pres));
  }
  ...
  // ─── SLIDE 3 — SOLUÇÃO ────────────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    darkSlide(s);
    sectionTag(s, "Solução", 0.38, pres);
  ...
  // ─── SLIDE 4 — MERCADO ────────────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    lightSlide(s);
    sectionTag(s, "Mercado", 0.38, pres);
    slideTitle(s, "Mercado grande e mal atendido", C.night);

    statBox(s, "450k+", "Clínicas no Brasil", 0.5, 1.7, pres);
    statBox(s, "R$2,4B", "TAM — gestão de clínicas", 2.65, 1.7, pres);
    statBox(s, "65%", "Sem sistema adequado", 4.8, 1.7, pres);
    statBox(s, "R$120M", "SAM — segmento-alvo", 6.95, 1.7, pres);
    ...
    s.addChart((pres as any).charts.BAR, [{
      name: "Clínicas (mil)",
  ...
  // ─── SLIDE 5 — MODELO DE NEGÓCIO ─────────────────────────────────────────────
  {
    const s = pres.addSlide();
    lightSlide(s);
    sectionTag(s, "Modelo de negócio", 0.38, pres);
    slideTitle(s, "SaaS recorrente com add-ons", C.night);

    planCard(s, "Solo", "R$197", ["1 profissional", "Agenda + prontuário", "Financeiro básico", "WhatsApp confirm."], 0.5, pres);
    planCard(s, "Clínica", "R$397", ["Até 5 profissionais", "Financeiro + NFS-e", "CRM + BI", "Gestão de equipe"], 3.45, pres, true);
    planCard(s, "Pro", "R$797", ["Ilimitados", "App mobile", "IA (voz + churn)", "Multi-unidade + API"], 6.4, pres);
  ...
  // ─── SLIDE 6 — TRAÇÃO ─────────────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    darkSlide(s);
    sectionTag(s, "Tração", 0.38, pres);
    slideTitle(s, "Crescimento consistente desde o beta");

    s.addChart((pres as any).charts.LINE, [{
      name: "MRR (R$)",
    ...
    kpis.forEach(([v, l], i) => statBox(s, v, l, 6.55, 1.55 + i * 1.0, pres, 2.9));
  }
  ...
  // ─── SLIDE 7 — ROADMAP ────────────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    lightSlide(s);
    sectionTag(s, "Roadmap", 0.38, pres);
    ...
    phases.forEach((p, i) => {
      ...
      if (i < 3) {
        s.addShape((pres as any).shapes.LINE, { x: x + 2.12, y: 2.35, w: 0.22, h: 0, line: { color: C.border, width: 1 } });
      }
    });
  ...
  // ─── SLIDE 8 — CONCORRÊNCIA ───────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    lightSlide(s);
    sectionTag(s, "Concorrência", 0.38, pres);
    ...
    rows.forEach((row, ri) => {
      ...
      row.forEach((cell, ci) => {
        ...
        s.addText(cell, {
          ...
          color: ci === 4 && cell === "✓" ? C.green : textColor,
          align: isHeader ? "left" : "center",
          valign: "middle",
          margin: isHeader ? [0, 0, 0, 6] : 0,
          bold: cell === "✓" || cell === "✗"
        });
      });
    });
  ...
  // ─── SLIDE 10 — CAPTAÇÃO ──────────────────────────────────────────────────────
  {
    const s = pres.addSlide();
    darkSlide(s);
    sectionTag(s, "Captação", 0.38, pres);
    slideTitle(s, "Rodada Seed — R$1,2M");

    s.addShape((pres as any).shapes.RECTANGLE, { x: 0, y: 0, w: 10, h: 5.625, fill: { color: C.night }, line: { color: C.night } });
    ...
    allocs.forEach((a, i) => {
      ...
      s.addShape((pres as any).shapes.RECTANGLE, { x, y, w: 0.06, h: 1.1, fill: { color: a.color }, line: { color: a.color } });
  ...
  // ─── SLIDE 11 — ENCERRAMENTO ─────────────────────────────────────────────────
  {
    ...
    s.addShape((pres as any).shapes.LINE, { x: 3.5, y: 2.8, w: 3, h: 0, line: { color: C.green, width: 1 } });
  ...
  await pres.writeFile({ fileName: "Clinora_PitchDeck.pptx" });
};
